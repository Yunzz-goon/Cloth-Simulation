...
double eval_pef(int n, int delta, double mass, double grav, double sep,
                ...
                double *fy, double *fz, int p) {
  double pe, rlen, xdiff, ydiff, zdiff, vmag;
  int nx, ny, dx, dy, i, j, k;
  double update_f_cons;
  // int chunk_size = n * n / ((2*delta+1)*(2*delta+1)*(p));
  // int chunk_size =1.5* n / ((2*delta+1));
...
//version 2
    ...
    for (i=0; i<2*delta+1; i++){
      for (j=0; j<2*delta+1; j++){
        ...
        nx, ny, dx, dy, rlen, xdiff, ydiff, zdiff, vmag, update_f_cons)\
        reduction(+:pe) schedule(static) collapse(2)
        #pragma ivdep
        for (nx = i; nx < n; nx+=delta2p1){
          for (ny = j; ny < n; ny += delta2p1)
          {
            ...
            // see what happened. 
            // printf("\t\tThread %d executes dx*n+dy: %d \n", omp_get_thread_num(), nx * n + ny);
